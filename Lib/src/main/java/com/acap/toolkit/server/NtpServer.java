package com.acap.toolkit.server;
 ...
 * <pre>
 * Tip:
 *      通过 NTP 授时服务器同步服务器时间 ，默认使用阿里云提供的 NTP 服务器
 *      通过 {@link NtpServer#synTime()} 开始同步时间
 *
 *
 * @author A·Cap
 * @date 2022/4/7 15:03
...
public class NtpServer {
    ...
    public void synTime() throws Throwable {
        try {
            ...
            mSocket.send(new DatagramPacket(data, data.length, InetAddress.getByName(mNtp.server), mNtp.port));

            DatagramPacket responsePacket = new DatagramPacket(data, data.length);
            mSocket.receive(responsePacket);

            double destinationTimestamp = System.currentTimeMillis() / 1000.0 + 2208988800.0;
            long time_end = SystemClock.elapsedRealtime() - time_start;
            NtpMessage responseMessage = new NtpMessage(responsePacket.getData());
            double pOffset = (responseMessage.receiveTimestamp - responseMessage.originateTimestamp + (responseMessage.transmitTimestamp - destinationTimestamp)) / 2;
    ...
    public static class Ntp {
        ...
        public Ntp(String server, int port) {
            this.server = server;
            this.port = port;
        }
    }
